...
To figure this out, I'll create a new polygon layer to represent a 30-meter buffer around the parking meters. st_buffer() will create a circle around each parking meter, but I'm adding st_union() after that to dissolve the overlapping circles and combine them all into one multipolygon. 
...
Now I'll join my two trees dataframes. For any trees that are in the full dataset, but not in the set of trees that are by parking meters, will have an NA value for the variables Name.y and Description.y, so I can use that to create a binary variable that indicates whether each tree is by a parking meter.
